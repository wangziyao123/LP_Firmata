...
class PyMata3:
    ...
    def i2c_read_request(self, address, number_of_bytes, read_type, register=None,
    ...
    def dht_config(self, pin_num):
        task = asyncio.ensure_future(self.core.dht_config(pin_num))
        self.loop.run_until_complete(task)

    def dht_Temperature(self):
        task = asyncio.ensure_future(self.core.dht_Temperature())
        value = self.loop.run_until_complete(task)
        return value

    def dht_Humidity(self):
        task = asyncio.ensure_future(self.core.dht_Humidity())
        value = self.loop.run_until_complete(task)
        return value

    def nfc_begin(self):
        task = asyncio.ensure_future(self.core.nfc_begin())
        self.loop.run_until_complete(task)

    def get_msg(self):
        task = asyncio.ensure_future(self.core.get_msg())
        self.loop.run_until_complete(task)

    def ir_config(self, pin_num):
        task = asyncio.ensure_future(self.core.ir_config(pin_num))
        self.loop.run_until_complete(task)

    def ir_getKey(self):
        task = asyncio.ensure_future(self.core.ir_getKey())
        value = self.loop.run_until_complete(task)
        return value