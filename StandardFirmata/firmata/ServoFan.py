...
def loop():
    ...
    for pos in range(0, 180, 10):  # Start=0 degrees, Finish=180 degree, (Increment=1 degree which is the default)
        board.analog_write(SERVO_PIN, pos)
        board.sleep(0.1)
    print("Servo sweep ( 180 to 0 degree )")
    sys.stdout.flush()
    for pos in range(180, 0, -10):  # Start=180 degrees, Finish=0 degrees, Increment=-1 degrees (moving down)
        board.analog_write(SERVO_PIN, pos)
        board.sleep(0.1)
