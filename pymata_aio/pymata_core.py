...
class PymataCore:
    ...
    def __init__(self, arduino_wait=4, sleep_tune=0.0001, log_output=False,
        ...
        self.command_dictionary = {PrivateConstants.REPORT_VERSION:
                                   ...
                                   PrivateConstants.PIXY_DATA:
                                       self._pixy_data,
                                   PrivateConstants.DFROBOT_MESSAGE:
                                       self._dfrobot_cmd_dispatcher}
        ...
        self.dfrobot_map = {
            "dht": {
                "config": False, "temperature": None, "humidity": None, "pin": None
            },
            "ir": {
                "config": False, "pin": None, "key": None
            },
            "nfc": {
                "config": False, "temperature": None, "humidity": None, "pin": None
            },
    ...
    async def i2c_read_request(self, address, number_of_bytes, read_type, register=None,
        ...
        while True:
            try:
                ...
                if next_command_byte == PrivateConstants.START_SYSEX:
                    ...
                    # print("from command_dispatch: ", sysex)
    ...
    async def _dfrobot_cmd_dispatcher(self, data):
        ...
        sub_msg = {
            ...
            PrivateConstants.SUB_MESSAGE_IR: self._ir_parser,
        }
        if data[0] in sub_msg:
            sub_msg[data[0]](data)

    async def dht_config(self, pin_num=None):
        if pin_num is None:
            if self.dfrobot_map['dht']['pin']:
                pin_num = self.dfrobot_map['dht']['pin']
            else:
                print('Need 1 argument for pin_number, but 0 argument give')
                return
        else:
            self.dfrobot_map['dht']['pin'] = pin_num
        command = [PrivateConstants.SUB_MESSAGE_DHT,
                   pin_num]
        # if not self.dfrobot_map['dht']['config']:
        #     self.dfrobot_map['dht']['config'] = True
        await self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, command)

    def _dht_parser(self, data):
        if not self.dfrobot_map['dht']['config']:
            self.dfrobot_map['dht']['config'] = True
        temp = data[2] + data[3] / 100
        temp = -temp if data[1] else temp
        hum = data[4] + data[5] / 100
        self.dfrobot_map['dht']['temperature'] = temp
        self.dfrobot_map['dht']['humidity'] = hum

    async def dht_Temperature(self):
        if self.dfrobot_map['dht']['config']:
            dht = self.dfrobot_map.get('dht')
            await self.dht_config()
            return dht['temperature']
        else:
            print("Please initialize the dht sensor first.")

    async def dht_Humidity(self):
        if self.dfrobot_map['dht']['config']:
            dht = self.dfrobot_map.get('dht')
            await self.dht_config()
            return dht['humidity']
    ...
    async def get_msg(self):
        print(self.dfrobot_map)

    async def nfc_begin(self):
        if not self.dfrobot_map['nfc']['config']:
            command = [PrivateConstants.SUB_MESSAGE_NFC,
                       PrivateConstants.SUB_MESSAGE_NFC_CONF]
            await self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, command)

    async def nfc_scan(self):
        command = [PrivateConstants.SUB_MESSAGE_NFC,
                   PrivateConstants.SUB_MESSAGE_NFC_SCAN]
        await self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, command)

    async def ir_config(self, pin_num=None):
        if pin_num is None:
            if self.dfrobot_map['ir']['pin']:
                pin_num = self.dfrobot_map['dht']['pin']
            else:
                print('Need 1 argument for pin_number, but 0 argument give')
                return
            ...
            self.dfrobot_map['dht']['pin'] = pin_num
        command = [PrivateConstants.SUB_MESSAGE_IR,
                   PrivateConstants.SUB_MESSAGE_IR_CONF, pin_num]
        if not self.dfrobot_map['dht']['config']:
            self.dfrobot_map['dht']['config'] = True
        await self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, command)

    async def ir_getKey(self):
        # if self.dfrobot_map['ir']['pin']:
        pin_num = self.dfrobot_map['dht']['pin']
        # else:
        #     print('Please run "ir_config" function first to set a pin as ir sensor.')
        #     return
        command = [PrivateConstants.SUB_MESSAGE_IR,
                   PrivateConstants.SUB_MESSAGE_IR_GET_KEY, pin_num]
        await self._send_sysex(PrivateConstants.DFROBOT_MESSAGE, command)
        await self.sleep(0.05)
        key = self.dfrobot_map['ir']['key']
        self.dfrobot_map['ir']['key'] = None
        if key == '0':
            key = None
        return key

    def _ir_parser(self, data):
        # if not self.dfrobot_map['ir']['config']:
        #     self.dfrobot_map['ir']['config'] = True
        key = 0
        for i in range(1, 6):
            key += (data[i]<<(7*(i-1)))
        self.dfrobot_map['ir']['key'] = hex(key)[2:]